...
class Reapear_model extends CI_Model {
	...
	{
		$this->db->where('id',$id);
		$update = $this->db->update('eligible_students_next_level',$form_array);
		return $update;
	...
	{		
		$query=$this->db->distinct()->select('COURSE')->get('eligible_students_next_level');
  			return $query->result_array();
	}

	public function getAllResult12()
	{						$this->db->order_by("id", "desc");
		$return_category = $this->db->get('eligible_students_next_level')->result_array();
		return $return_category;
	}
	...
	public function getAllResult1($month,$year,$course)
	{
		...
		$this->db->order_by('id', 'desc');
		$query = $this->db->get('eligible_students_next_level')->result_array();
