...
function checkBoxValidationSeller(req,AdmEmail) {	
	
	if (req == 'Processed') {

		var ship_from = $('#pickup_location').val();
		if(ship_from == '') {
	...
	if(tot == 0) {
		...
		confirm_global_seller_order_status(req,AdmEmail);
	} 
		
}
function confirm_global_seller_order_status(req,AdmEmail){
 	$.confirm({
 		'title'		: 'Confirmation',
 		'message'	: 'Do you want move the order to '+req+ '?',
 		'buttons'	: {
 			'Yes'	: {
 				'class'	: 'yes',
 				'action': function(){
 					$('.myModal_popup').modal(true)
...
function orderCancelByUser(dealCodeNumber){
 	$.confirm({
 		'title'		: 'Confirmation',
 		'message'	: 'Do you want cancel the order?',
 		'buttons'	: {
 			'Yes'	: {
 				'class'	: 'yes',
 				'action': function(){
...
function checkQuestionValid(){
	...
	if(question ==''){
		...
		$('#div_question').show().delay('3000').fadeOut();
		return false;
	}
	if(question.length<10){
		$('#div_question').html('Type message minimum 10 characters');
		$('#div_question').show().delay('3000').fadeOut();
		return false;
	}
	else{
		...
		$.ajax({
		type: 'POST',   
		 url: baseURL+'site/product/insertEditQuestion',
		data:{"question_title":question,"questioned_product_id":product_id},
			success:function(response){
				//alert(response);
				if(response == 'Success'){
...
function checkAnswerValid(){
	...
	var answer_question_id=$('#answer_question_id').val();
	var product_id=$('#product_id').val();
	if(answer ==''){
		$('#div_answer').html('This field is required');
		return false;
	}
	if(answer.length<10){
		$('#div_answer').html('Type message minimum 10 characters');
		$('#div_answer').show().delay('3000').fadeOut();
		return false;
	}
	else{
		$('#div_answer').html('');
		...
		$.ajax({
		type: 'POST',   
		 url: baseURL+'site/product/insertEditAnswer',
		data:{"answers_title":answer,"answer_question_id":answer_question_id,"questioned_product_id":product_id},
			success:function(response){
				//alert(response);
				if(response == 'Success'){
...
function bootsrating(){

	document.getElementById('PetVoteRate').innerHTML = '<span class="red">Please Login for feedback</span>' ;	
}
...
function validateSeller_Signup(){
      ...
      //  var pattern = /(ftp|http|https):\/\/(\w+:{0,1}\w*@)?(\S+)(:[0-9]+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?/;
		...
		$('#seller_username_Err').html('This field required');
		}
		...
		}else if(seller_first_name==0){ 
		$('#seller_firstname_Err').html('This field required');	
		...
		}else if(seller_banner==undefined){ 
		$('#seller_bann_Err').html('This field required');	
						...
						{	
							if(response.success == 0) {
								...
								if(response.msg=='User name not valid'){
									$('#usernameErr').show();
									$('#usernameErr').html('User name not valid. Only alphanumeric allowed');
									$('#loadErr').html('');
									return false;
								}
...
function save_for_later_user(val,cid,selid) {
		$.ajax({
			type: 'POST',   
			url:baseURL+'site/cart/ajaxSaveForLater',
			data:{'curval':val,'cart':'usercart','sellId':selid},
			success:function(response){
				window.location.reload();
			}
...
function move_to_cart(val,cid,selid) {
		$.ajax({
			type: 'POST',   
			url:baseURL+'site/cart/ajaxMoveCart',
			data:{'curval':val,'cart':'usercart','sellId':selid},
			success:function(response){
				window.location.reload();
			}
...
/* Get frequently bought together product */

function ajax_fbp_add_cart(){
	
	
	$('#ADDCartErr').html('');
	$('#ADDCartErr').show()
var sell_id=$('#sell_id').val();
	var fbp_pro_ids=$('.fbp_final_ids').val();
	$.ajax({
		type: 'POST',
		url: baseURL+'site/cart/userfbpcartadd',
		data: {'product_id':fbp_pro_ids, 'sell_id':sell_id},
		success: function(response){
				//alert(response);
				...
				/*if(arr[0] =='login'){
					...
					/*if($.isNumeric(arr[1])==true){
						$('#ADDCartErr').html('<span class="red">Maximum Purchase Quantity: '+mqty+'. Already in your cart: '+arr[1]+'</span>');
					}else{
						$('#ADDCartErr').html('<span class="red">'+arr[1]+'.</span>');
					}*/					
						$('#ADDCartErr').show().delay('2000').fadeOut();
				}else{
					$('#CartCount').html(arr[1]);
					$('#ADDCartErr').html('<span class="green">Product Added in your cart</span>');
					$('#ADDCartErr').show().delay('2000').fadeOut();
					/*$('#MiniCartViewDisp').html(arr[1]);*/
					$('.cart_full_bg').show();
					location.reload();
					/*$('.cart_full_bg').css('background-image',"url(<?php echo base_url();?>images/ajax-loader(14).gif)");*/
					$('#CartCount #cart_popup').show();
...
function paypal(){
	...
	$('.creditcard').removeClass('current');
	$('.cod').removeClass('current');
	$('.stripe').removeClass('current');
	$('.payu').removeClass('current');
}
...
function othermethods(){
	...
	$('.othermethods').addClass('current');		
	
}

function home_type(){
	$('.add_home').addClass('current');
	$('.add_work').removeClass('current');
	var home = "1";
	$('.add_type').val(home);
}
function work_type(){
	$('.add_work').addClass('current');
	$('.add_home').removeClass('current');
	var work = "2";
	$('.add_type').val(work);	
}
function edit_home_type(){
	$('.add_edit_home').addClass('current');
	$('.add_edit_work').removeClass('current');
	var edit_home = "1";
	$('#add_type').val(home);
}
function edit_work_type(){
	$('.add_edit_work').addClass('current');
	$('.add_edit_home').removeClass('current');
	var edit_work = "2";
	$('#add_type').val(work);	
}
...
$(document).ready(function(e) {
	...
	$('#variation_one').change(function(e) {
		...
		var_price = $(this).find('option:selected').data('price');
		variation_price = $(this).find('option:selected').data('dprice');
		var_col_opt = $(this).find('option:selected').data('dcolor');
		
		//alert(variation_price);
		if(variation_price!=''){
			//var newamt=variation_price;
			//var dicount_amt=$('#dicount_amt').val();
			//var price=Number(newamt)-((Number(newamt)/100)*Number(dicount_amt));
			$('#price').val(var_price);						
			$('#variation_one_val').val(var_col_opt);
			$('#view-price').html(variation_price);
...
function validate_create_list(val)
{
	...
	var list_namestr=$('#creat_list_'+val).val();
	var list_name = myTrim(list_namestr);
	//$("form").preventDefault();
	if(list_name !='')
	{
		/* var ddl=$('#ddl').val();
		var list=$('#list').val();
		var productId=$('#productId').val(); alert(productId);
		$.ajax({
		type:'post',
		url:baseURL+'site/user/add_list',
		data:{'ddl':ddl,'list':list_name,'productId':productId},
			success: function(response){
				//alert(response);
				window.location.href = window.location.pathname;
            	}
...
function validate_create_list1(val)
{
	...
	if(list_name !='')
	{
		...
		//alert(ddl+' - '+list_name+' - '+productId);
		$.ajax({
		type:'post',
		url:baseURL+'site/user/add_list_user',
		data:{'ddl':ddl,'list':list_name,'productId':productId},
...
/*Auto suggest*/
/*$(document).ready(function(e) {
    $('.home_product_search').keyup(function(e) {
		if($('.home_product_search').val()==''){$('.search_sugglist').html(''); return false;}
        search_key=$(this).val();
		$.ajax({
...
$(document).ready(function(e) {
    $('.home_product_search_btm').keyup(function(e) {
		if($('.home_product_search_btm').val()==''){$('.search_sugglist_btm').html(''); return false;}
        search_key=$(this).val();
		$.ajax({
			url : baseURL+'site/search/autosuggest_list/'+search_key,
			data : {'search_key' : search_key},
...
function rattingValidation(){
	...
	if (title == '') {
		$('#titleErr').html('Please enter the review title');
		$('#titleErr').show().delay('3000').fadeOut();
		return false;
	}
	else
	{
